...
def TSEB_PT(Tr_K,
    ...
    Tr_K = np.asarray(Tr_K).astype(np.float32)
    (vza,
     T_A_K,
     u,
     ea,
    ...
    if const_L is None:
        # Initially assume stable atmospheric conditions and set variables for
        L = np.asarray(np.zeros(Tr_K.shape, np.float32) + np.inf).astype(np.float32)
        max_iterations = ITERATIONS
    else:  # We force Monin-Obukhov lenght to the provided array/value
        L = np.asarray(np.ones(Tr_K.shape, np.float32) * const_L).astype(np.float32)
        max_iterations = 1  # No iteration
    # Calculate the general parameters
    rho = met.calc_rho(p, ea, T_A_K)  # Air density
    c_p = met.calc_c_p(p, ea)  # Heat capacity of air
    ...
    # Calculate LAI dependent parameters for dataset where LAI > 0
    omega0 = CI.calc_omega0_Kustas(LAI, f_c, x_LAD=x_LAD, isLAIeff=True)
    F = np.asarray(LAI / f_c).astype(np.float32)  # Real LAI
    ...
    # iteration of the Monin-Obukhov length
    u_friction = MO.calc_u_star(u, z_u, L, d_0, z_0M)
    u_friction = np.asarray(np.maximum(U_FRICTION_MIN, u_friction)).astype(np.float32)
    ...
    # radiometric T
    T_C = np.asarray(np.minimum(Tr_K, T_A_K)).astype(np.float32)
    ...
    for n_iterations in range(max_iterations):
        ...
        alpha_PT_rec = np.asarray(alpha_PT + 0.1).astype(np.float32)
        while np.any(LE_S[i] < 0):
            ...
            H[i] = np.asarray(H_C[i] + H_S[i]).astype(np.float32)
            LE[i] = np.asarray(LE_C[i] + LE_S[i]).astype(np.float32)
            # Now L can be recalculated and the difference between iterations
            # derived
            if const_L is None:
                L[i] = MO.calc_L(
                ...
                u_friction[i] = np.asarray(np.maximum(U_FRICTION_MIN, u_friction[i])).astype(np.float32)
...
def _L_diff(L, L_old):
    ...
    return L_diff.astype(np.float32)
...
def calc_F_theta_campbell(theta, F, w_C=1, Omega0=1, x_LAD=1):
    ...
    return np.asarray(ftheta).astype(np.float32)
...
def calc_H_C(T_C, T_A, R_A, rho, c_p):
    ...
    return np.asarray(H_C).astype(np.float32)
...
def calc_H_DTD_parallel(
    ...
    return np.asarray(H).astype(np.float32)
...
def calc_H_S(T_S, T_A, R_A, R_S, rho, c_p):
    ...
    return np.asarray(H_S).astype(np.float32)
...
def calc_T_C(T_R, T_S, f_theta):
    ...
    return np.asarray(flag), np.asarray(T_C).astype(np.float32)
...
def calc_T_C_series(Tr_K, T_A_K, R_A, R_x, R_S, f_theta, H_C, rho, c_p):
    ...
    return np.asarray(T_C).astype(np.float32)
...
def calc_T_CS_Norman(F, vza_n, vza_f, T_n, T_f, w_C=1, x_LAD=1, omega0=1):
    ...
    return np.asarray(T_C_K).astype(np.float32), np.asarray(T_S_K).astype(np.float32)
...
def calc_T_S(T_R, T_C, f_theta):
    ...
    return np.asarray(flag).astype(np.int32), np.asarray(T_S).astype(np.float32)
...
def calc_T_S_4SAIL(T_R, T_C, rdot_star, emiss_v_eff, emiss_s_eff, L_dn=0):
    ...
    return np.asarray(flag), np.asarray(T_S).astype(np.float32)
...
def calc_T_S_series(Tr_K, T_A_K, R_A, R_x, R_S, f_theta, H_S, rho, c_p):
    ...
    return np.asarray(T_S).astype(np.float32), np.asarray(T_AC).astype(np.float32)


def _check_default_parameter_size(parameter, input_array):

    parameter = np.asarray(parameter).astype(np.float32)
    if parameter.size == 1:
        parameter = np.ones(input_array.shape) * parameter
        return np.asarray(parameter).astype(np.float32)
...
def calc_resistances(res_form, res_types):
    ...
    R_A = np.asarray(np.maximum(1e-3, R_A)).astype(np.float32)
    R_x = np.asarray(np.maximum(1e-3, R_x)).astype(np.float32)
    R_S = np.asarray(np.maximum(1e-3, R_S)).astype(np.float32)
